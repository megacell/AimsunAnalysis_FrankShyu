...
def extractSingleDB(fileName, thisPercentage, debug, maxT, minT):
  #----------------------------------------------------------#
  ...
  cur = con.cursor()
  cur.execute('SELECT oid, entranceTime, (exitTime - entranceTime), origin, destination FROM MIVEHTRAJECTORY WHERE exitTime != -1 AND entranceTime > {} AND entranceTime < {}'.format(minT, maxT))
  ...
  for i in xrange(int(math.ceil((maxT - minT) / timeStepSize))):
    ...
    if i in tStepMinTime:
      ...
      #absNash.append(tStepMinTime[i])
      rltNash.append(100*(tStepAccumTime[i]/(tStepAccumCount[i] * tStepMinTime[i]) - 1))
    else:
      absNash.append(0)
      rltNash.append(0)
      count += 1
...
def traverseMultiDB(fileList, debug, maxT, minTime):
  ...
  for filename in fileList:
    ...
    thisTStep, thisAbsNash, thisRltNash = extractSingleDB(filename, thisPercentage, debug, maxT, minTime)
...
if __name__ == '__main__':
  ...
  outputDir  = getOutputDir(motherDir, maxTime, minTime, '/timeND/')
  ...
  percentage, timeStep, absNash = averageTimeND(outputDir)
  generatePlot(percentage, timeStep, absNash, '../outputFigures/', True, 2*3600, 4*3600)
